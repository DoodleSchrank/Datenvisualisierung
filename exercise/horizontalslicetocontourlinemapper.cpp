...
HorizontalSliceToContourLineMapper::mapSliceToContourLineSegments() {
     ...
     */
    for (float iso : c_) vectors.append(MarchingSquares(iso));
    return vectors;
}
...
#pragma omp critical
            {
                vectors.append(QVector3D(x1 / (dim_ - 1), y1 / (dim_ - 1), 0.001));
                vectors.append(QVector3D(x2 / (dim_ - 1), y2 / (dim_ - 1), 0.001));

                // if special case
                if ((!v3 && v2 && !v1 && v0) || (v3 && !v2 && v1 && !v0)) {
                    vectors.append(QVector3D(x / (dim_ - 1), (y + 0.5) / (dim_ - 1), 0.001));
                    vectors.append(QVector3D((x + 0.5) / (dim_ - 1), (y + 1) / (dim_ - 1), 0.001));
                }
            }
        }
    }
